...
def execute(request: str):
    ...
    guess_result = yield llm(
        "Ask the user to guess the number. Do **NOT** guess a number on behalf of the user; "
    ...
    if won:
        yield auto(
            ...
            context=(
                'If the user guessed the right number, print out "You got it right!"; '
                'otherwise, print out "You got it wrong!"'
            ),
        )
    else:
        yield auto(
