...
- **Editing mekara skills**: When asked to edit a mekara skill, ALWAYS edit the natural language source file under `.agents/skills/` (e.g., `.agents/skills/finish/SKILL.md`), NOT the compiled Python version inside the skill folder. Never edit the auto-generated compiled `mekara.py` files directly unless you are fixing pre-commit hook errors.
  - After editing the source, tell the user to run the compilation process to regenerate the Python script IF there is already an existing `.agents/skills/<skill>/mekara.py`. Sources that have not yet been compiled do not need to be compiled.
- **Wiki holds generic skills, `.agents/skills/` holds mekara-specific skills**: `docs/wiki/` and `src/mekara/bundled/skills/` contain the generic versions of skills that work for any project using mekara. `.agents/skills/` may contain customized overrides for this repo (e.g., `project/release` has mekara-specific PyPI steps). Skills listed in `docs/docs/code-base/mekara/bundled-script-generalization.md` are excluded from sync in both directions — wiki/bundled and `.agents` are maintained independently for those skills.
- **Adding new mekara hooks**: When you add a new hook command to `src/mekara/cli.py` (e.g., `mekara hook auto-approve`), you MUST also update `.agents/skills/ai-tooling/setup-mekara-mcp/SKILL.md` to include that hook in the installation instructions. This ensures users who run `/ai-tooling:setup-mekara-mcp` get the complete hook configuration automatically.
...
- **ALWAYS skip Python tests for non-Python changes**: If your changes do not affect Python runtime behavior (for example: Markdown docs, Docusaurus config/content, `.agents/skills/*/SKILL.md` skill sources), do NOT run `poetry run pytest`. This is a waste of time. Rely on the repo's pre-commit hooks to run formatting and docs build checks during commit.
...
**Code vs documentation**: Code can have inline comments that reference the documentation, but the docs are the ultimate source of truth. For systematized skills (`.agents/skills/*/SKILL.md`), inline reasoning is appropriate since these are natural language scripts—see the systematize command for details.
...
- **Follow `.agents/skills/*/SKILL.md` instructions literally**: If the user says "follow the instructions in `.agents/skills/<name>/SKILL.md`", treat that skill file as the task spec and do _exactly_ what it describes (including its output artifacts and stop points). Do not "helpfully" implement code unless the skill explicitly instructs you to, or the user separately asks you to implement.
  - **Planning commands are not implementation**: If the referenced command is a planning command (for example, `.agents/skills/plan-design-doc/SKILL.md`), produce the design artifact it describes (for example, a design doc under `docs/docs/roadmap/`) and stop to get user approval before making implementation changes.
...
- **Start with the simplest implementation**: Before adding abstractions, CLI interfaces, helper functions, or indirection, ask: "Does this need to exist?" If a function is only called once, inline it. If a script is only invoked from one place, merge them. If a CLI flag has no external callers, remove it. Add complexity only when there’s a proven need, not "just in case."
- **Verify before removing or changing**: Before removing a section, feature, or file, check whether it’s still used or referenced. Don’t assume something is obsolete because the thing that motivated it changed — the thing itself may still serve a purpose independently.
- **Ask when you don’t understand, don’t muddle through**: If you don’t fully understand how something works (conflict detection semantics, sync directions, who calls what), ask instead of writing vague or misleading descriptions. A wrong answer you’re confident about wastes more time than a question.
...
- **Confirm before operational changes**: Do not change public APIs, workflows, or basic testing fundamentals without explicitly checking with the user first. For example, this means not having tests write to the local `.agents/skills/` folder just because you think it makes more sense to output new Git-tracked files in tests than to mock file operations.
...
- **Don't broaden scope during commit prep**: If the user has confirmed a commit, do not add generated-file updates, prompt rewrites, or other extra edits just because a hook or test exposes follow-on work. Treat scope expansion as a blocker: stop at the first blocker, explain exactly what additional changes would be required, and do not commit unless the user explicitly approves the expanded scope.
- **VCR mismatches require re-recording, not prompt rewrites**: If a prompt or command output legitimately changed and a VCR replay fails, stop and ask the user to re-record the cassette. Do not “fix” the mismatch by changing generated prompt text away from the intended source behavior.
...
- **After context compaction, respond to the user's last message**: "Continue from where you left off" means resume the conversation at the point it was interrupted — which means responding to the last message, not skipping past it to execute tasks. If the last message was a question, answer it _without_ executing any task actions -- questions are to be _answered_, not acted on. If it was an instruction mid-task, follow the instruction instead of continuing on with the task as-is. Do not skip the last message.
