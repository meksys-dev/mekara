...
- **Editing mekara scripts**: When asked to edit a mekara script, ALWAYS edit the natural language source file under `.agents/skills/` (e.g., `.agents/skills/finish/SKILL.md`), NOT the compiled Python version under `.mekara/scripts/compiled/`. Never edit the auto-generated compiled `.py` files directly unless you are fixing pre-commit hook errors.
...
- **Wiki holds generic scripts, `.mekara/scripts/nl/` holds mekara-specific scripts**: `docs/wiki/` and `src/mekara/bundled/scripts/nl/` contain the generic versions of scripts that work for any project using mekara. `.mekara/scripts/nl/` may contain customized overrides for this repo (e.g., `project/release.md` has mekara-specific PyPI steps). Scripts listed in `docs/docs/code-base/mekara/bundled-script-generalization.md` are excluded from sync in both directions — wiki/bundled and `.mekara` are maintained independently for those scripts.
  - Note: Compiled-only edits (`src/mekara/bundled/scripts/compiled/`) don't trigger the bundled script validation hook.
