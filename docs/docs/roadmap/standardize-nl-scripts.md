...
**Key rule**: Never destroy information. "Remove a section" always means fold its content into a standard section (usually Key Principles or the appropriate step body), not delete it.
:::
...
- [ ] `merge-main.md` — remove `# Merge Main` H1; renumber steps 1–7 → 0–6; fold `## Special Cases` into the appropriate step body; fold `## Resolution Strategy` into the appropriate step body; fold `## Example` and `## Verification Checklist` content into Key Principles; convert numbered Key Principles to bullets
- [ ] `project/worktree-init.md` — fold `## Rollback Plan` content into Step 2 body or Key Principles
- [ ] `rsi-scripting.md` — fold `## Example` content into `## Key Principles` (rename/repurpose the section)
- [ ] `systematize.md` — remove `## Context` section (fold key sentence into opening description or Step 0); fold `## Example Transformation` content into Key Principles; update `src/mekara/bundled/scripts/nl/systematize.md` preserving generalizations
